<div class="p-4 md:p-6 max-w-xl mx-auto">
    ...
    <div class="mb-6">
        <div class="flex items-center space-x-3 mb-2">
            ...
            <h1 class="text-2xl font-bold text-gray-800">New Unit</h1>
        </div>
        <p class="text-gray-600 ml-13">Add a new measurement unit</p>
    </div>
    ...
    <div class="ios-card p-6 md:p-8 shadow-sm mb-8">
        <form wire:submit.prevent="save" class="space-y-6">
            ...
            <!-- Common Units Suggestion -->
            <div class="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl p-4 border border-blue-200">
                <h4 class="text-sm font-medium text-blue-800 mb-3">Common Unit Suggestions</h4>
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    @foreach (['Kilogram', 'Gram', 'Liter', 'Milliliter', 'Piece', 'Meter', 'Centimeter', 'Box', 'Pack', 'Dozen'] as $commonUnit)
                        <button type="button" wire:click="$set('name', '{{ $commonUnit }}')"
                            class="text-sm px-3 py-2 rounded-lg border border-blue-200 bg-white hover:bg-blue-50 text-blue-700 transition-colors">
                            {{ $commonUnit }}
                        </button>
                    @endforeach
                </div>
    ...
    <div
        ...
        <div class="max-w-xl mx-auto flex items-center justify-end space-x-3">
            ...
            <button type="submit" wire:click="save" wire:loading.attr="disabled"
                class="bg-gradient-to-r from-blue-500 to-cyan-500 text-white px-6 py-3 rounded-xl font-medium shadow-md hover:shadow-lg transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] disabled:opacity-70 disabled:cursor-not-allowed flex items-center space-x-2 text-sm">
    ...
    @if ($processing)
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
            <div class="bg-white rounded-2xl p-6 max-w-xs mx-4 text-center">
                ...
                <h3 class="text-lg font-semibold text-gray-800 mb-2">Creating Unit</h3>
